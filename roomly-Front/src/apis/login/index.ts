...
import { GET_GUEST_SIGN_IN, GUEST_SIGN_IN_API_URL, HOST_SIGN_IN_API_URL, PATCH_GUEST_PASSWORD_API_URL, PATCH_GUEST_TEL_NUMBER_API_URL, PATCH_HOST_PASSWORD_API_URL, PATCH_HOST_TEL_NUMBER_API_URL } from 'src/constants';
...
import PatchGuestAuthRequestDto from '../signUp/dto/request/guest/patch-guest-auth.request.dto';
import { PatchHostTelNumberRequestDto } from '../signUp/dto/request';
...
// function: 게스트 전화번호 수정및 삭제 처리 함수//
export const  ChangeGuestPhonNumberRequest = async(guestId: string, requestBody: PatchGuestAuthRequestDto, accessToken: string) => {

    const responseBody = await axios.patch(PATCH_GUEST_TEL_NUMBER_API_URL(guestId), requestBody, bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
}

// function: 호스트 전화번호 수정및 삭제 처리 함수//
export const  ChangeHostPhonNumberRequest = async(hostId: string, requestBody: PatchHostTelNumberRequestDto, accessToken: string) => {

    const responseBody = await axios.patch(PATCH_HOST_TEL_NUMBER_API_URL(hostId), requestBody, bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
}
...
export const ChangeHostPwRequest = async (userId: string, requestBody: HostPwChangeRequestDto, accessToken: string) => {
    
    const responseBody = await axios.patch(PATCH_HOST_PASSWORD_API_URL(userId), requestBody, bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
};

...
import { guestIdFind, guestIdFindAuth, guestPwFind, telAuthCheckRequest, telAuthRequest } from 'src/apis/signUp';
...
import { GUEST_ID_FIND_API_URL, GUEST_ID_FIND_TEL_AUTH_CHECK_API_URL, PATCH_GUEST_PASSWORD_FIND_API_URL } from 'src/constants';
import GuestIdFindRequestDto from 'src/apis/signUp/dto/request/guest/guest-id-find-request.dto';
import GuestIdFindSuccessResponseDto from 'src/apis/login/dto/response/guest-id-find-success-response-dto';
import GuestPwFindRequestDto from 'src/apis/signUp/dto/request/guest/guest-pw-find-request.dto';
...
export default function FindId() {
    ...
    const onPasswordCheckChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const isGuestPasswordMatch = guestPw && guestPw === value;
    ...
    const onSubmitGuestPasswordChangeHandler = async () => {
        try {
            ...
            // const response = await axios.patch(PATCH_GUEST_PASSWORD_FIND_API_URL, data);
            
            const requestBody: GuestPwFindRequestDto = {
                guestId,
                guestPw // 속성의 이름과 담을 변수의 이름이 동일한 경우 하나로 작성
            }
            guestPwFind(requestBody).then(telAuthResponse);

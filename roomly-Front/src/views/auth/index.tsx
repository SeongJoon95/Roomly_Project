...
import GuestTelAuthCheckRequestDto from 'src/apis/signUp/dto/request/guest/guest-tel-auth-check-request.dto';
import GuestTelAuthRequestDto from 'src/apis/signUp/dto/request/common/tel-auth.request.dto';
import TelNumberRequestDto from 'src/apis/signUp/dto/request/common/tel-auth.request.dto';
...
function SnsContainer({ type }: SnsContainnerProps) {
  ...
  const onSnsButtonClickHandler = (sns: 'kakao' | 'google') => {
    window.location.href = `http://localhost:4000/api/v1/auth/guest/sns-sign-in/${sns}`;
  };

  // render : SNS 로그인 회원가입 컴포넌트 렌더링 //
  return (
    <div className="sns-container">
      <div className="snsMessage">SNS {type}</div>
...
// variable: 1. 회원가입 가능 여부 (게스트)
const guestAllFieldsFilled = guestName && guestId && guestPassword && guestPasswordCheck && telNumber && isAgreed && isCheckedId && isMatchedPassword
&& isCheckedPassword && isSend && authNumber && isCheckedAuthNumber;
...
// variable: 2. 회원가입 가능 여부 (호스트)
  ...
  const onGuestSignUpButtonHandler = () => {
    ...
    if (!guestAllFieldsFilled) return;

    const requestBody: GuestSignUpRequestDto = {
      guestName, // guestName 변수를 사용
      guestId, // guestId 변수를 사용
      guestPw: guestPassword, // guestPassword 변수를 사용
      snsId, // guestSnsId 변수를 사용
      ...
      <div style={{ paddingTop: '50px' }}>
        ...
        <div className={`${currentView}-input-box-signup`}>
          ...
          <div className={`${currentView}-button-container2`}>
            ...
            <button className={`${currentView}-button-clear-${guestAllFieldsFilled ? 'active' : 'disable'}`} 
            onClick={currentView === 'guest' ? onGuestSignUpButtonHandler : onHostSignUpButtonHandler}>회원가입</button>

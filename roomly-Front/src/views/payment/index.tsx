        ...
        IMP.request_pay(
            ...
            async function (rsp: { success: boolean; error_msg?: string, differenceinTime: number }) {
                ...
                if (rsp.success) {
                    ...
                    try {
                        await axios.post("http://localhost:4000/api/roomly/reservation/createReservation", {
                            totalPrice: calculatePricePerNight(),
                            totalNight: differenceInTime(),
                            checkInDate: checkInTime,
                            checkOutDate: checkOutTime,
                            reservationTotalPeople: Number(personnelCount),
                            createdAt: dayjs().format('YYYY-MM-DD HH:mm:ss'),
                            guestId: signInUser?.guestId,
                            roomId: roomId
                        });

                        // 결제 성공 후 메인 페이지로 리다이렉션

...
import { ChangeGuestPwRequest, ChangeHostPhonNumberRequest, ChangeHostPwRequest } from 'src/apis/login';
...
import PatchHostTelNumberRequestDto from 'src/apis/signUp/dto/request/host/patch-host-tel-auth.request.dto';
import { HostTelAuthRequestDto, TelNumberRequestDto} from 'src/apis/signUp/dto/request';
import PatchGuestAuthRequestDto from 'src/apis/signUp/dto/request/guest/patch-guest-auth.request.dto';
...
export default function Information({ titletext, username, activite }: Props) {
    ...
    const onGuestPasswordChangeHandler = async () => {
        const hostAccessToken = cookies[HOST_ACCESS_TOKEN];
            ...
            // const requestBody: PatchHostPasswordRequestDto = {
                currentHostPw: currentPassword,
                changeHostPw: hostPassword
            };

            ChangeHostPwRequest(hostId, requestBody, hostAccessToken).then(passwordChangeResponse);
        }
    ...
    // event handlerr: 전화번호 변형버튼 클릭핸들러 
    const telNumberChangeHandler = () => {
        setTelNumberInputActive(false)
        setTelNumber('')
    }

    // event handler: 인증번호 발송
    const onClickTelnumberAuthHandler = () => {
        ...
        // HostTelAuthRequestDto: 이거 사용 x
        // const requestBody: HostTelAuthRequestDto = {
        const requestBody: TelNumberRequestDto = {
            telNumber
        }
    ...
    // event handler: 인증번호 확인 및 전화번호 변경
    const onGuestTelNumberChangeHandler = () => {

        const hostAccessToken = cookies[HOST_ACCESS_TOKEN];

        const requestBody: PatchHostTelNumberRequestDto = {
            telNumber,
            authNumber
        }
        ChangeHostPhonNumberRequest(hostId, requestBody, hostAccessToken).then(telAuthCheckResponse);
    }
    ...
    alert('전화번호가 변경 되었습니다. 다시 로그인 해주세요.');
    navigator('/main')
    removeCookies('hostAccessToken')
        ...
        <>
            {activite && <div id='information-warpper'>
                ...
                <div className='information-main'>
                    ...
                    onButtonClick={telNumberChangeHandler} />}

                    {!telNumberInputActive && (
                        <MypageInputBox
                            activation={true}
                            title="전화번호"
                            type="text"
                            ...
                            onChange={(e) => setTelNumber(e.target.value)} // 전화번호 전용 핸들러
                    ...
                    {telAuthActivite && (
                        <MypageInputBox
                            ...
                            onChange={(e) => setAuthNumber(e.target.value)} // 인증번호 전용 핸들러
                            messageError={authNumberMessage}
                            buttonName="변경하기"
